...
export async function fetchKeyOverviewQuota(signal?: AbortSignal): Promise<KeyOverviewQuota> {
  const response = await apiFetch(apiPath('/key-overview/quota'), { signal, cache: 'no-store' })
  if (!response.ok) {
    await parseApiError(response, `Failed to load key overview quota: ${response.status}`)
  }
  return (await response.json()) as KeyOverviewQuota
}
